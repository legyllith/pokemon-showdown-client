...
class BattleTextParser {
	...
	static upgradeArgs({args, kwArgs}: {args: Args, kwArgs: KWArgs}): {args: Args, kwArgs: KWArgs} {
		...
		case '-activate': {
			...
			if ([
				...
				'electricterrain', 'mistyterrain', 'psychicterrain', 'burningterrain', 'telepathy', 'stickyhold', 'suctioncups', 'aromaveil',
	...
	parseArgsInner(args: Args, kwArgs: KWArgs) {
		...
		case '-fail': {
			...
			templateId = 'fail';
			if (['brn', 'frz', 'par', 'psn', 'slp', 'drw', 'frb', 'substitute'].includes(id)) {
